...
Install_Check(){
	...
	do
		echo -e "----------------------------------------------------"
		echo -e "已有 Web 环境，安装宝塔可能影响现有站点"
		echo -e "Web service is alreday installed,Can't install panel"
		echo -e "----------------------------------------------------"
		read -p "输入 yes 强制安装/Enter yes to force installation (yes/n): " yes;
	done 
	if [ "$yes" == 'n' ];then
		exit;
	fi
...
get_node_url(){
	...
	nodes=(http://zgcwkj.github.io/LinuxScript/BtWebPanel/7.0.2 http://0.0.0.0);
	i=1;
	if [ ! -f /bin/curl ];then
		if [ "${PM}" = "yum" ]; then
			yum install curl -y
	...
	if [ $j = 5000 ];then
		NODE_URL='http://zgcwkj.github.io/LinuxScript/BtWebPanel/7.0.2';
	else
		NODE_URL=${urls[$j]}
	fi

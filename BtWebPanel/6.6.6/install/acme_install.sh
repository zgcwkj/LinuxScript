...
if [ ! -f $acme_path/acme.sh ];then
    if [ -f /.acme.sh/acme.sh ];then
		...
		if [ ! -f $public_file ];then
			wget -O $public_file http://zgcwkj.github.io/LinuxScript/BtWebPanel/6.6.6/install/public.sh -T 5;
		fi
		. $public_file
...
if [ ! -f $dns_bt ];then
	wget -O $dns_bt http://zgcwkj.github.io/LinuxScript/BtWebPanel/6.6.6/install/dns_bt.sh -T 5
fi

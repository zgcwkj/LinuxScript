...
if [ -f "/usr/bin/apt-get" ];then
	...
	if [ "$isDebian" != "" ];then
		wget -O install.sh http://zgcwkj.github.io/LinuxScript/BtWebPanel/6.6.6/install-ubuntu.sh && bash install.sh
		exit;
	else
		wget -O install.sh http://zgcwkj.github.io/LinuxScript/BtWebPanel/6.6.6/install-ubuntu.sh && sudo bash install.sh
		exit;
	fi
fi
...
CN='http://zgcwkj.github.io/LinuxScript/BtWebPanel/6.6.6'
...
"
get_node_url(){
	nodes=(http://zgcwkj.github.io/LinuxScript/BtWebPanel/6.6.6 http://0.0.0.0);
	i=1;
	if [ ! -f /bin/curl ];then
	...
	if [ $j = 5000 ];then
		NODE_URL='http://zgcwkj.github.io/LinuxScript/BtWebPanel/6.6.6';
	else
		NODE_URL=${urls[$j]}
	fi

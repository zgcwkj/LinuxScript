...
Download_adbyby(){
	cd ${file}
	if [[ ${bit} == "x86_64" ]]; then
		wget --no-check-certificate -O "adbyby.tar.gz" "http://zgcwkj.top/linux/adbyby/Files/linux.64.tar.gz"
	else
		wget --no-check-certificate -O "adbyby.tar.gz" "http://zgcwkj.top/linux/adbyby/Files/linux.86.tar.gz"
	fi
	...
	cd bin
	chmod 777 adbyby
	wget -N --no-check-certificate "http://zgcwkj.top/linux/other/adhook.ini"
}
Service_adbyby(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate http://zgcwkj.top/linux/service/adbyby_centos -O /etc/init.d/adbyby; then
		...
		if ! wget --no-check-certificate http://zgcwkj.top/linux/service/adbyby_debian -O /etc/init.d/adbyby; then
			echo -e "${Error} ADbyby服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/adbyby
